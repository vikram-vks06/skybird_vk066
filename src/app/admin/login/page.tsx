...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6" style={{ backgroundColor: '#0F1F3D' }}>
      <div className="absolute inset-0 opacity-10 pointer-events-none">
        <div className="absolute top-1/4 left-1/3 w-96 h-96 rounded-full" style={{ background: 'radial-gradient(circle, #E8A020 0%, transparent 70%)' }} />
        <div className="absolute bottom-1/4 right-1/3 w-80 h-80 rounded-full" style={{ background: 'radial-gradient(circle, #2A7FD4 0%, transparent 70%)' }} />
      </div>
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-white/30 text-xs uppercase tracking-widest mt-3 font-semibold">Admin Portal</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-4xl p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <input
                type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@skybirds.in"
        ...
        <p className="text-center text-white/20 text-xs mt-6">
          <Link href="/" className="hover:text-white/40 transition-colors">← Back to website</Link>
        </p>
      </motion.div>
    </div>
  );
