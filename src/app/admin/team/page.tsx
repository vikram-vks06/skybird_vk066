...
interface TeamMember { _id: string; name: string; designation: string; bio: string; imageUrl?: string; isActive: boolean; order: number; accentColor?: string; }
...
export default function AdminTeamPage() {
  ...
  const toggleActive = async (item: TeamMember) => {
    const res = await fetch(`/api/team/${item._id}`, {
    ...
    <div className="max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/admin/team/new" className="px-5 py-2.5 rounded-full text-white text-sm font-bold" style={{ backgroundColor: '#0F1F3D' }}>+ Add New</Link>
        </div>

        {items.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 shadow-card text-center">
            <p className="text-navy/40">No team members yet.</p>
          </div>
        ) : (
            ...
            {items.map((item) => (
              <div key={item._id} className="bg-white rounded-2xl p-5 shadow-card flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-medium" style={{ color: item.accentColor || '#2A7FD4' }}>{item.designation}</p>
                  <p className="text-xs text-navy/40 mt-1 line-clamp-2">{item.bio}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  ...
                  <Link href={`/admin/team/${item._id}`} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-sky-brand/10 text-sky-brand hover:bg-sky-brand/20 transition-colors">Edit</Link>
